...
from app.common.utils.jwt_utils import admin_jwt_manager
from app.common import log_util

logger = log_util.get_logger("admin_jwt_middleware")
...
class AdminJWTMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if self._is_excluded_path(path):
            logger.debug(f"跳过 JWT 验证（排除路径） | 路径: {path} | 方法: {method}")
            return await call_next(request)

        if not self._check_route_auth_requirement(request):
            logger.debug(f"跳过 JWT 验证（装饰器标记） | 路径: {path} | 方法: {method}")
            return await call_next(request)

        authorization: str = request.headers.get("Authorization", "")

        if not authorization:
            logger.warning(f"缺少 Authorization 头 | 路径: {path} | 方法: {method} | IP: {client_ip}")
            return self._create_auth_error_response("缺少认证令牌")

        token = self._extract_token_from_header(authorization)
        if not token:
            logger.warning(f"无效的 Authorization 格式 | 路径: {path} | 方法: {method} | IP: {client_ip}")
            return self._create_auth_error_response("无效的认证令牌格式")

        # 使用 admin_jwt_manager 验证 token
        payload = admin_jwt_manager.verify_token(token)
        if not payload:
            logger.warning(f"Admin JWT 令牌验证失败 | 路径: {path} | 方法: {method} | IP: {client_ip}")
        ...
        logger.info(
            f"Admin JWT 认证成功 | 用户ID: {user_id} | 用户名: {payload.get('username')} | 路径: {path} | 方法: {method}")
